...
namespace GenerationLogBook.Controllers
{
    ...
    public class BQIController : Controller
    {
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_MASTER_DATA_MGMT_BQ1)]
        public ActionResult MasterDataListForm()
        {
            IEnumerable<UnitsMaster> model = _context.UnitsMasters.Where(u => u.SiteId == CommDef.SITE_KEY_BQ1 && u.StatusId == "A");
        ...
        public JsonResult FetchTemperatureLedger(List<LossInputSet> input)
        {
            ...
            TemperatureFormViewModel vmodel = temphandler.FetchTemperatureLedger(date, CommDef.SITE_KEY_BQ1);
            vmodel.Controller = "BQI";
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_SAFETY_BQ1_F1)]
        public ActionResult OpenSafetyForm()
        {
            SafetyFormViewModel vmodel = new SafetyFormViewModel("BQI", CommDef.SITE_KEY_BQ1);
        ...
        public JsonResult FetchSafetyLedger(List<LossInputSet> input)
        {
            ...
            SafetyFormViewModel vmodel = new SafetyFormViewModel(_context, _userId, _authId, CommDef.SITE_KEY_BQ1, date, "BQI");
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_BQ1_PWRAPPROVAL)]
        public ActionResult ElossL3ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_BQ1_PWRAPPROVAL;
            BQIElossFormViewModel vmodel;
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_BQ1_PERAPPROVAL)]
        public ActionResult ElossL2ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_BQ1_PERAPPROVAL;// "ELOSBQPS IL2";
            BQIElossFormViewModel vmodel;
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_BQ1_OPRAPPROVAL)]
        public ActionResult ElossL1ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_BQ1_OPRAPPROVAL;// "ELOSKPCL1";
            BQIElossFormViewModel vmodel;
            ...
            vmodel.hiddenCols = new List<string> { "L2Approvalbool", "ForcedDeration", "PlannedDeration" };

            return View("ElossDataEntryForm", vmodel);
        }

        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_BQ1_DATAENTRY)]
        public ActionResult ElossDataEntryForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_BQ1_DATAENTRY;// "ELOSKPCDE";
            BQIElossFormViewModel vmodel;
            ...
            vmodel.hiddenCols = new List<string> { "L1Approvalbool", "L2Approvalbool", "ForcedDeration", "PlannedDeration" };

            return View(vmodel);
        }

        private BQIElossFormViewModel buildElossFormViewModel(string AuthId)
        {
            BQIElossFormViewModel vmodel = new BQIElossFormViewModel();

            vmodel.UserId = _userId;
            vmodel.AuthId = AuthId;

            vmodel.LOVUnits = (from res in _context.UnitsMasters
                               where res.SiteId == CommDef.SITE_KEY_BQ1
            ...
            vmodel.LOVReason = new Utility.LovManager().getPreDefinedRasonsBySiteID(CommDef.SITE_KEY_BQ1);
        ...
        public JsonResult FetchLossLedger(List<LossInputSet> input)
        {
            ...
            vmodel.FormHeading = "BQPS I-Energy Loss Recording Register [" + UnitName + ", " + date.ToString("dd-MM-yyyy") + "]";
        ...
        private List<UnitReadingRegister1> getLedger(string pUnitId, DateTime pdate)
        {
            ...
            switch (_authId)
            {
                case CommDef.AUTH_ENERGYLOSS_BQ1_DATAENTRY:
                case CommDef.AUTH_ENERGYLOSS_BQ1_PWRAPPROVAL:
                    res = _context.FetchRegister1(pUnitId, pdate).OrderBy(z => z.RdgDateTime);
                    break;
                case CommDef.AUTH_ENERGYLOSS_BQ1_OPRAPPROVAL:
                    res = _context.FetchRegister1(pUnitId, pdate).Where(z => z.RecSrc == "DB").OrderBy(z => z.RdgDateTime);
                    break;
                case CommDef.AUTH_ENERGYLOSS_BQ1_PERAPPROVAL:
        ...
        private void AddRecord(UnitReadingRegister1 rec)
        {
            ...
            //rec.Reason = this.getPreDefinedRasonById(rec.Reason == null ? "N" : rec.Reason);
            //implementing requirement 2 changes
