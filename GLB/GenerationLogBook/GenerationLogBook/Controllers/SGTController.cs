...
namespace GenerationLogBook.Controllers
{
    ...
    public class SGTController : Controller
    {
        ...
        public JsonResult FetchTemperatureLedger(List<LossInputSet> input)
        {
            ...
            TemperatureFormViewModel vmodel = temphandler.FetchTemperatureLedger(date, CommDef.SITE_KEY_SGT);
            vmodel.Controller = "SGT";
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_SAFETY_SGT_F1)]
        public ActionResult OpenSafetyForm()
        {
            SafetyFormViewModel vmodel = new SafetyFormViewModel("SGT", CommDef.SITE_KEY_SGT);
        ...
        public JsonResult FetchSafetyLedger(List<LossInputSet> input)
        {
            ...
            SafetyFormViewModel vmodel = new SafetyFormViewModel(_context, _userId, _authId, CommDef.SITE_KEY_SGT, date, "SGT");
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_MASTER_DATA_MGMT_SGT)]
        public ActionResult MasterDataListForm()
        {
            IEnumerable<UnitsMaster> model = _context.UnitsMasters.Where(u => u.SiteId == CommDef.SITE_KEY_SGT && u.StatusId == "A");
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_SGT_PWRAPPROVAL)]
        public ActionResult ElossL3ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_SGT_PWRAPPROVAL;
            SGTELossFormViewModel vmodel;
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_SGT_PERAPPROVAL)]
        public ActionResult ElossL2ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_SGT_PERAPPROVAL;
            SGTELossFormViewModel vmodel;
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_SGT_OPRAPPROVAL)]
        public ActionResult ElossL1ApprovalForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_SGT_OPRAPPROVAL;
            SGTELossFormViewModel vmodel;
        ...
        [GenLogBookAuthorization(ReqiredAuth = CommDef.AUTH_ENERGYLOSS_SGT_DATAENTRY)]
        public ActionResult ElossDataEntryForm()
        {
            string AuthId = CommDef.AUTH_ENERGYLOSS_SGT_DATAENTRY;
            SGTELossFormViewModel vmodel;
        ...
        private SGTELossFormViewModel buildElossFormViewModel(string AuthId)
        {
            SGTELossFormViewModel vmodel = new SGTELossFormViewModel();

            vmodel.UserId = _userId;
            vmodel.AuthId = AuthId;

            vmodel.LOVUnits = (from res in _context.UnitsMasters
                               where res.SiteId == CommDef.SITE_KEY_SGT
            ...
            vmodel.LOVReason = new Utility.LovManager().getPreDefinedRasonsBySiteID(CommDef.SITE_KEY_SGT);
        ...
        public JsonResult FetchLossLedger1(List<LossInputSet> input)
        {
            ...
            vmodel.FormHeading = "SGTPS-Energy Loss Recording Register [" + UnitName + ", " + date.ToString("dd-MM-yyyy") + "]";
        ...
        private List<UnitReadingRegister1> getLedger1(string pUnitId, DateTime pdate)
        {
            ...
            switch (_authId)
            {
                case CommDef.AUTH_ENERGYLOSS_SGT_DATAENTRY:
                case CommDef.AUTH_ENERGYLOSS_SGT_PWRAPPROVAL:
                    res = _context.FetchRegister1(pUnitId, pdate).OrderBy(z => z.RdgDateTime);
                    break;
                case CommDef.AUTH_ENERGYLOSS_SGT_OPRAPPROVAL:
                    res = _context.FetchRegister1(pUnitId, pdate).Where(z => z.RecSrc == "DB").OrderBy(z => z.RdgDateTime);
                    break;
                case CommDef.AUTH_ENERGYLOSS_SGT_PERAPPROVAL:
            ...
            foreach (var x in res)
            {
                ledger.Add(new UnitReadingRegister1
                {
                    ...
                    ActualDispatchMW = x.RecSrc == "DB"? x.ActualDispatchMW:(decimal) 8.6,
